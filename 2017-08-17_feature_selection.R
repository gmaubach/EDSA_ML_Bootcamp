...
# Load libraries
library(entropy)  # for mi.plugin
library(fpc)      # for bhattacharyy.dist
...
ct_income = max(sapply(
  1:3, 
  function(order) {
    cor.test(careers$income ^ order, 
             careers$prestige)$estimate
  }
  ))

ct_education = max(sapply(1:3, function(order)
  cor.test(careers$education ^ order, careers$prestige)$estimate))
...
# A singleton class is a class with only one value = case
# in it.
careers$income
summary(careers$income)

careers$income=(careers$income-1)%/%25
careers$education=(careers$education-1)%/%25

# Look at the values
print(careers$income)
print(careers$education)
...
# We can look at the values
cddScores
# INTERPETATION: Higher values are better - as with correlation.


# Now we bin the target prestige variable
# I use the %/% operator, which is integer division.
summary(careers$prestige)
print(careers$prestige)
...
# Source: https://de.wikipedia.org/wiki/Transinformation
